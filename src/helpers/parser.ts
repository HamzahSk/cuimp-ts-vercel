...
const findBinaryWithVersion = (browser: string, version: string): string | null => {
  // Use tmp directory for binaries (Vercel compatibility)
  const tmpDir = os.tmpdir()
  const homeBinariesDir = path.resolve(tmpDir, '.cuimp', 'binaries')
...
const downloadAndExtractBinary = async (
  ...
  try {
    ...
    // Use tmp directory for binaries (Vercel compatibility)
    const tmpDir = os.tmpdir()
    const binariesDir = path.resolve(tmpDir, '.cuimp', 'binaries')
